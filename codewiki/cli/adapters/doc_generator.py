...
from codewiki.cli.utils.errors import APIError, ConfigurationError

# Import backend modules
from codewiki.src.be.documentation_generator import DocumentationGenerator, GenerationResult
...
class CLIDocumentationGenerator:
    ...
    def __init__(
        ...
        # Setup job metadata from config_manager
    ...
    def generate(self) -> DocumentationJob:
        ...
        try:
            # Load settings from CLI config via ConfigManager
            try:
                settings = self.config_manager.load_settings_for_backend(
                    repo_path=str(self.repo_path), output_dir=str(self.output_dir)
                )
            except ConfigurationError:
                # Re-raise ConfigurationError as-is (already user-facing)
                raise
            except (ValueError, AssertionError) as e:
                # Convert validation errors to ConfigurationError with context
                raise ConfigurationError(
                    f"Configuration validation failed for repo '{self.repo_path}' "
                    f"with output '{self.output_dir}': {e}"
                ) from e
            except Exception as e:
                # Catch unexpected errors during config loading
                raise ConfigurationError(
                    f"Failed to load configuration for repo '{self.repo_path}' "
                    f"with output '{self.output_dir}': {e}"
                ) from e

            # Run backend documentation generation
            gen_result = asyncio.run(self._run_backend_generation(settings))
            ...
            # Complete job based on generation result
            _ = time.time() - start_time  # Track time (may be used for logging later)

            if gen_result.partial_success:
                # Some modules failed but generation continued
                failed_modules = gen_result.failed_leaf_modules + gen_result.failed_parent_modules
                self.job.partial_complete(
                    warning_message=gen_result.status_message,
            ...
            self.job.fail(f"Configuration error: {e}")
            raise
    ...
    async def _run_backend_generation(self, settings: "Settings") -> GenerationResult:
        ...
        # Create documentation generator with new signature
        ...
        try:
            ...
            from codewiki.core import FIRST_MODULE_TREE_FILENAME, MODULE_TREE_FILENAME
            ...
            try:
                if os.path.exists(first_module_tree_path):
                    ...
                    module_tree = await cluster_modules(
                        leaf_nodes,
                        components,
                        doc_generator.settings,
                        doc_generator.resilient_client,
                        doc_generator.logger,
                    )
            ...
            try:
                ...
                gen_result = await doc_generator.generate_module_documentation(components, leaf_nodes)
            ...
            # Ensure clean shutdown of async resources
            if hasattr(doc_generator, "resilient_client") and doc_generator.resilient_client:
                await doc_generator.resilient_client.close()
    ...
    def _run_html_generation(self):
        ...
        # Validate and provide defaults for repo_info
        required_keys = ["name", "url", "github_pages_url"]
        missing_keys = [k for k in required_keys if k not in repo_info]
        if missing_keys:
            self.progress_tracker.update_stage(0.5, f"Warning: Missing repo info: {missing_keys}")

        # Provide defaults for missing or None values
        repo_info.setdefault("name", self.repo_path.name)
        repo_info.setdefault("url", "")
        repo_info.setdefault("github_pages_url", "")

        # Also handle None values (keys present but value is None)
        if repo_info["name"] is None:
            repo_info["name"] = self.repo_path.name
        if repo_info["url"] is None:
            repo_info["url"] = ""
        if repo_info["github_pages_url"] is None:
            repo_info["github_pages_url"] = ""
