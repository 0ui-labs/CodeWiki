...
class AgentOrchestrator:
    ...
    def __init__(self, settings: Settings, repo_path: str, max_depth: int = 2):
        """
        Initialize the AgentOrchestrator.

        Args:
            settings: Core settings for LLM configuration
            repo_path: Path to the repository being documented
            max_depth: Maximum depth for module tree hierarchy
        """
        self.settings = settings
        self.repo_path = repo_path
        self.max_depth = max_depth
        self.logger = get_logger(settings)
        self.model = CodeWikiModel(settings, self.logger)
    ...
    async def process_module(self, module_name: str, components: Dict[str, Node],
        ...
        deps = CodeWikiDeps(
            ...
            absolute_repo_path=str(os.path.abspath(self.repo_path)),
            registry={},
        ...
        if os.path.exists(docs_path):
            self.logger.info(f"✓ Module docs already exists at {docs_path}")
            return module_tree
        ...
        try:
            ...
            self.logger.error(f"Error processing module {module_name}: {str(e)}", module=module_name)
            raise