...
from .config import WebAppConfig, get_web_app_settings
...
class BackgroundWorker:
    ...
    def load_job_statuses(self):
        ...
        try:
            ...
            print(
                f"Loaded {len([j for j in self.job_status.values() if j.status == 'completed'])} completed jobs from disk"
            )
    ...
    def _process_job(self, job_id: str):
        ...
        doc_generator = None  # Initialize for cleanup in finally block
        ...
        try:
            ...
            # Load fresh settings for THIS job (isolated instance)
            settings = get_web_app_settings()
            job.main_model = settings.main_model
            ...
            # Prepare output directories for this job
            job_docs_dir = os.path.join("output", "docs", f"{job_id}-docs")
            dependency_graph_dir = os.path.join("output", "dependency_graphs")

            job.progress = "Generating documentation..."

            # Create FRESH DocumentationGenerator instance for this job
            doc_generator = DocumentationGenerator(
                settings=settings,
                repo_path=temp_repo_dir,
                output_dir=job_docs_dir,
            ...
            try:
                ...
                # Use ResilientLLMClient.close() which delegates to the inner LLMClient
                if hasattr(doc_generator, "resilient_client") and doc_generator.resilient_client:
                    loop.run_until_complete(doc_generator.resilient_client.close())
                loop.close()

            # Cache the results
            docs_path = os.path.abspath(job_docs_dir)
            ...
            # Cleanup temporary repository (cross-platform)
            if "temp_repo_dir" in locals() and os.path.isdir(temp_repo_dir):
                try:
                    shutil.rmtree(temp_repo_dir)
                except Exception:
                    logger.exception("Failed to cleanup temp directory: %s", temp_repo_dir)
